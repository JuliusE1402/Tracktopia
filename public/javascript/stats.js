const orderedList = document.getElementById("most-streamed-tracks");
const timeSpanSelection = document.getElementById("time-span-selection");
...
timeSpanSelection.selectedIndex = localStorage.getItem("selectionIndex");

if (localStorage.getItem("statSelection") == "tracks") {
	window.addEventListener("load", getInfoTracks());
} else {
	window.addEventListener("load", getInfoArtists());
}

timeSpanSelection.addEventListener("change", handleSelectChange);
...
function handleSelectChange(event) {
	...
	if (selectedOption == "lifetime") {
		option = "long_term";
		timeSpanSelection.selectedIndex = 0;
	} else if (selectedOption == "6 months") {
		option = "medium_term";
		timeSpanSelection.selectedIndex = 1;
	} else {
		option = "short_term";
		timeSpanSelection.selectedIndex = 2;
	}

	// save the selected index to local storage
	localStorage.setItem("selectionIndex", timeSpanSelection.selectedIndex);

	// send an HTTP GET request to the server with the selected option as a query parameter
	fetch(`/stats?timeSpan=${option}`)
		.then((response) => {
			// redirect the user to the stats page after the server returns a response
			window.location.href = `${baseUrl}stats?timeSpan=${option}`;
		})
		.catch((error) => {
...
function getInfoTracks() {
	orderedList.innerHTML = ""; // remove all existing elements from the ordered list
	localStorage.setItem("statSelection", "tracks");
	searchBar.setAttribute("placeholder", "search for tracks");
	fetch(baseUrl + "favoriteTracks", {
		method: "GET",
	})
		...
		.then((data) => {
			for (let i = 0; i < data.length; i++) {
				...
				listItem.innerHTML = `<p>${i + 1}</p><a href="${
					data[i].url
				}" target="_blank"><img src="${data[i].image}"></a>
            <h4>${data[i].title}</h4>
            <h6>${data[i].artist}</h6>
            <audio-player src="${
							data[i].previewLink
						}"  bar-width="5" bar-gap="2" crossorigin></audio-player>`;
...
function getInfoArtists() {
	localStorage.setItem("statSelection", "artists");
	orderedList.innerHTML = ""; // remove all existing elements from the ordered list
	searchBar.setAttribute("placeholder", "search for artists");

	fetch(baseUrl + "favoriteArtists", {
		method: "GET",
	})
		.then((response) => response.json())
		.then((data) => {
			for (let i = 0; i < data.length + 50; i++) {
				...
				listItem.innerHTML = `<p>${i + 1}</p><a href="${
					...
					data[i].image
				}" class="artists-tab-box" height="155px"></a>
            <h4>${data[i].name}</h4>`;
				orderedList.appendChild(listItem);
			}
		})
...
searchBar.addEventListener("keyup", function (event) {
	...
	const trackTitles = document.getElementsByTagName("h4");
	for (let i = 0; i < trackTitles.length; i++) {
		const titleText = trackTitles[i].textContent.toLowerCase();
		if (titleText.includes(searchText)) {
			trackTitles[i].parentNode.style.display = "block";
		} else {
			trackTitles[i].parentNode.style.display = "none";
		}
	}
});

tracksButton.addEventListener("click", getInfoTracks);
artistsButton.addEventListener("click", getInfoArtists);
